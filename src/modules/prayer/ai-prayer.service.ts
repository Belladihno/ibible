...
export class AiPrayerService {
    ...
    async rephrasePrayerRequest(prayerRequest: string, type: PrayerType): Promise<string>{
        try {
            ...
            if (!result || !result.trim()) {
                ...
                throw new HttpException('Empty rephrase result from AI', HttpStatus.INTERNAL_SERVER_ERROR);
            }
            ...
            cleaned = cleaned.replace(/^Here is a rephrased version[\s\S]*?:\s*/i, '');
            ...
            this.logger.error('Failed to rephrase prayer request', error?.message ?? error);
            throw new HttpException(
                'Failed to rephrase prayer request', HttpStatus.INTERNAL_SERVER_ERROR
            )
        }
    }


    async generatePrayer(rephrasedRequest: string, type: PrayerType): Promise<string>{
         try {
        ...
        if (!result || !result.trim()) {
            ...
            throw new HttpException('Empty generated prayer from AI', HttpStatus.INTERNAL_SERVER_ERROR);
        }
     ...
     private buildRephrasePrompt(prayerRequest: string, type: PrayerType): string {
        const prayerTypeText = type === PrayerType.SELF ? 'personal prayer' : 'prayer for others';
    ...
    private buildPrayerGenerationPrompt(rephrasedRequest: string, type: PrayerType): string {
    const prayerTypeText = type === PrayerType.SELF ? 'personal' : 'intercessory';
