...
export class PrayerService {
    ...
    async createPrayer(createPrayerDto: CreatePrayerDto, userId: string): Promise<{ tempId: string; originalRequest: string; rephrasedRequest: string; type: string }>{
   ...
   async confirmAndGeneratePrayer(prayerId: string, finalRequest?: string, userId?: string): Promise<Prayer> {
        const logger = new Logger('PrayerService');

        // Try to find an existing persisted prayer first
        const prayer = await this.prayerRepository.findOne({ where: { id: prayerId } });

        // If no persisted prayer, look for a temporary prayer (created via POST /prayers)
        if (!prayer) {
            const temp = await this.tempPrayerRepository.findOne({ where: { id: prayerId } });
            if (!temp) throw new NotFoundException('Prayer not found');
            if (userId && temp.userId !== userId) throw new NotFoundException('Prayer not found');

            const rephrased = temp.rephrasedRequest || temp.originalRequest;
            const source = (finalRequest && finalRequest.trim().length) ? finalRequest : rephrased;

            logger.debug(`Confirming temp prayer and generating AI for source length=${source.length}`);

            const aiPrayer = await this.aiPrayerService.generatePrayer(source, temp.type as any);
        ...
        const source = (finalRequest && finalRequest.trim().length)
            ? finalRequest
            : prayer.rephrasedRequest || prayer.originalRequest;

        logger.debug(`Generating prayer for source length=${source.length}`);

        prayer.aiPrayer = await this.aiPrayerService.generatePrayer(source, prayer.type);
